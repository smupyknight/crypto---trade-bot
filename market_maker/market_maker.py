...
class OrderManager:
    def __init__(self):
        ...
        self.running_entry = position['avgEntryPrice']
    ...
    def print_status(self):
        ...
        logger.info("Contracts Traded This Run: %d" % (self.starting_qty - self.running_qty))
    ...
    def place_orders(self):
        ...
        if position['avgEntryPrice'] == 0:
            ...
            self.running_entry = 0
            self.starting_qty = 0
            self.running_qty = 0
            return

        # Check if the position was modified.
        if abs(self.running_entry - position['avgEntryPrice']) / self.running_qty > 0.01:
            logger.info('The position is modified. Reset all params.')
            self.exchange.cancel_bulk_orders(existing_orders)
            existing_orders = []
            self.running_qty = self.starting_qty = self.exchange.get_delta()
            self.running_entry = position['avgEntryPrice']
            
        pending_vol = sum([o['leavesQty'] for o in existing_orders if o['side'] == 'Sell'])
        remaining_vol = self.starting_qty * settings.POSITION_PERCENTAGE - (self.starting_qty - self.running_qty) - pending_vol
        if remaining_vol == 0:
            return

        # Check the current profit level
        # If the profit is larger than 1%, place orders at PROFIT_LEVEL+delta
        # If not, place orders at ENTRY+delta
        current_pl = (self.start_position_mid - self.running_entry) / self.running_entry
        logger.info('Current profit: {:.2f}'.format(current_pl))
        if current_pl > settings.PROFIT_THRESHOLD:
            self.start_position_sell = self.running_entry * (1.0002 + settings.PROFIT_LEVEL)
        else: # Price falling, getting out at even.
            self.start_position_sell = min(self.start_position_sell, self.running_entry * 1.001)
            to_cancel = []
            # Cancel orders at the profit level
            for o in existing_orders:
                if o['price'] >= self.running_entry * (1 + settings.PROFIT_THRESHOLD):
                    to_cancel.append(o)
                    existing_orders.remove(o)
                    remaining_vol += o['orderQty']
            if to_cancel:
                self.exchange.cancel_bulk_orders(to_cancel)

        logger.info('Start selling at {:.2f}'.format(self.start_position_sell))
        logger.info('Remaining quantity(excluding placed orders): {}'.format(remaining_vol))
        buy_orders = []
        sell_orders = []
        ...
        if len(to_create) > 0:
            new_vol = sum([o['orderQty'] for o in to_create if o['side'] == 'Sell'])
            if remaining_vol < new_vol:
                for o in to_create:
                    o['orderQty'] = int(remaining_vol / len(to_create))

            if remaining_vol < settings.ORDER_START_SIZE:
                min_price = min([o['price'] for o in to_create])
                to_create[0]['price'] = math.toNearest(min_price, self.instrument['tickSize'])
                to_create[0]['orderQty'] = remaining_vol
